...
## Task 3: ChemBERTa эмбеддинги молекул
...
def _chunk_pool(mat: np.ndarray, m: int) -> np.ndarray:
    """(L, H) -> (m, H): усреднение по m примерно равным чанкам вдоль L."""
    if len(mat) == 0:
        return np.zeros((m, mat.shape[1] if mat.ndim == 2 else 0), dtype=np.float32)
    idx = np.array_split(np.arange(len(mat)), m)
    return np.stack([mat[chunk].mean(0) if len(chunk) else mat.mean(0)
                     for chunk in idx]).astype(np.float32)
    ...
    if todo:
        ...
        for i in range(0, len(items), batch_size):
            ...
            for j, (k, _) in enumerate(chunk):
                ...
                tokens[k] = _chunk_pool(valid, m_tokens)
...
```bash
git add src/features/molecule.py tests/test_molecule_embed.py
git commit -m "ChemBERTa эмбеддинги молекул (mean-pool + M-токены, кэш)"
```
...
В начало файла добавить импорт `os` (если ещё нет) и вспомогательные функции;
затем новую функцию. Вставить ПОСЛЕ существующей `embed_sequences`:
```python
def _chunk_pool_residues(mat: np.ndarray, m: int) -> np.ndarray:
    """(L, D) -> (m, D): усреднение по m примерно равным чанкам остатков."""
    idx = np.array_split(np.arange(len(mat)), m)
    ...
    if todo:
        ...
        for i in range(0, len(items), batch_size):
            ...
            for j, (k, v) in enumerate(chunk):
                ...
                cached[k] = _chunk_pool_residues(res, m_tokens)
        cache_file.parent.mkdir(parents=True, exist_ok=True)
        tmp = cache_file.with_name(cache_file.name + ".tmp")
        with open(tmp, "wb") as f:
            np.savez(f, **cached)
...
Примечание: отдельные по-типные модели (H3) и XGBoost по типу добавляются как
расширение оркестратора после подтверждения базового прогона единой модели —
на этапе интерпретации (Step 5). Базовый оркестратор выше даёт единую модель с
тремя слияниями (H4) по типам и в целом.
...
Затем **дополнить оркестратор для H3**: обучить отдельные по-типные two-tower
(белки-only, молекулы-only, переиспользуя `src/models/twotower.py` с pooled-
эмбеддингами) и XGBoost по типу; добавить их строки в `part2_metrics.csv`.
Сравнить по-типные метрики единой модели с отдельными (H3) и три слияния (H4).
...
- **Покрытие спецификации:** молекулы SMILES → Task 2; ChemBERTa → Task 3;
  M-токены белков → Task 4; объединённые пары (оговорённые оставлены) → Task 5;
  сборка с маршрутизацией → Task 6; три слияния → Task 7; единая модель →
  Task 8; оркестратор + прогон + H3/H4 → Task 9. Конфиг/окружение → Tasks 0,1.
- **Плейсхолдеры:** отсутствуют; код приведён полностью. Отдельные по-типные
  модели и XGBoost по типу (H3-планки) явно вынесены в Step 5 Task 9 как
  расширение оркестратора — переиспользуют готовые `twotower.py`/`baseline.py`.
